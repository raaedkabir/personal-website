  ...
  <main>
    <div class="container pt-120">
      <div class="top">
        ...
        <h1 class="title heading__secondary">Generative Art with HTML5 Canvas</h1>
      </div>
      <img src="@/assets/images/works/generativeArt/heading.png" alt="" />
      <div class="write-up">
        <div class="write-up__container">
          ...
          <canvas id="c1" />
          <p>
            In tune with the title of this page, "Generative Art", it's time to add some randomness! Let's make the
            circles smaller and randomly show half of them with <code>Math.random() > 0.5</code>.
          </p>
          <canvas id="c2" />
          <p>
            Now let's also make the sizes of the circles random too! But instead of using the usual
            <code>Math.random()</code> which gives us a random number from 0 (inclusive) to 1 (exclusive) we will use
            the Gaussuan (or Normal) distribution. Gaussian distributions show up very often in nature and will give us
            a more organic randomness.
          </p>
          <canvas id="c3" />
          <p>
...
export default {
  mounted() {
    ...
    if (canvas4) {
      ...
      const palette = _.chain(palettes)
        .sample()
        .shuffle()
        .value();
